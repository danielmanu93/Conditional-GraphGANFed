...
class CriticConv(nn.Module):
    def __init__(self, in_features, out_feature_list, b_dim, dropout):
        ...
        self.linear1 = nn.Linear(in_features, out_feature_list[0])
        self.linear2 = nn.Linear(out_feature_list[0], out_feature_list[1])
        self.linear3 = nn.Linear(out_feature_list[1], out_feature_list[2])
    ...
    def forward(self, input, adj, activation=None):
        ...
        # output = torch.stack([self.linear3(output) for _ in range(adj.size(1))], 1)
        # print(adj.shape, output.shape)
        # output = torch.einsum('bijk,bikl->bijl', (adj, output))
        # print("7:", output.shape)
        # # print("6:", hidden.shape)
        # output = torch.sum(output, 1) + self.linear3(output)
